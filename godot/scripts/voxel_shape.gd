...
static func is_shape_connected(voxels: Array) -> bool:
	if voxels.is_empty():
		return false
	var set: Dictionary = to_set(voxels)
	var q: Array = [voxels[0]]
	var seen: Dictionary = {voxels[0]: true}
...
static func legal_removals(voxels: Array) -> Array:
	...
	for c in surface_voxels(voxels):
		var t: Array = voxels.duplicate()
		t.erase(c)
		if not t.is_empty() and is_shape_connected(t):
			legal.append(c)
	return legal
